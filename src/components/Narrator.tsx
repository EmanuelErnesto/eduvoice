...
export const Narrator = forwardRef<NarratorHandle, {}>((_, ref) => {
  ...
  // Delays irregulares para criar um efeito de onda orgânico
  ...
  return (
    <div className="flex flex-col items-center justify-center mb-6 w-full h-24 relative">
      {/* Container da Animação de Voz */}
      <div 
        className={`absolute inset-0 flex items-center justify-center gap-2 transition-all duration-300 ${isSpeaking ? 'opacity-100 scale-100' : 'opacity-0 scale-90'}`}
        ...
        {animationDelays.map((delay, i) => (
          <div
            ...
            style={{
              height: '100%',
              maxHeight: '60px',
              animation: isSpeaking ? `audioWave 0.8s ease-in-out infinite` : 'none',
      ...
      {/* Ícone Estático (Modo Silêncio) */}
      <div 
        className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ${!isSpeaking ? 'opacity-100 scale-100' : 'opacity-0 scale-90'}`}
      >
